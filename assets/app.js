...
function setupMusicToggle(){
  ...
  function setState(playing, disabled=false){
    ...
    btn.title = disabled ? 'Agrega música desde el panel admin' : (playing ? 'Música ON' : 'Música OFF');
    btn.setAttribute('aria-label', btn.title);
    if(icon) icon.textContent = disabled ? '🎵' : (playing ? '🔊' : '🔇');
    if(label) label.textContent = disabled ? 'Sin música' : (playing ? 'Música ON' : 'Música OFF');
  }
  ...
  btn.onclick = async ()=>{
    ...
    try{
      ...
      setState(false);
      alert('El navegador bloqueó la reproducción. Toca nuevamente el botón de música.');
    }
  };
